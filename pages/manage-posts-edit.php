      ...
      <?php require dirname(__DIR__) . "/parts/error_box.php"; ?>
        <form 
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control" id="post-content" rows="10" name="content"><?php echo $post['content']; ?>
            </textarea>
          </div>
          <div class="mb-3">
            <label for="post-content" class="form-label">Status</label>
            <select class="form-control" id="post-status" name="status">
              <option value="pending review" <?php echo ( $post['status'] == 'review' ? 'selected' : '' ); ?>>Pending for Review</option>
              <option value="publish" <?php echo ( $post['status'] == 'review' ? 'selected' : '' ); ?>>Publish</option>
            </select>
          </div>
          <div class="text-end">
